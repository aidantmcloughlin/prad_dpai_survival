...
class GlobavgData(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        wsi_features_stack = torch.stack(wsi_features, dim=0)
        ## apply PCA, if specified:
        if not (self.dataset_cfg.pc_dim is None or self.dataset_cfg.pc_dim == {}):
            wsi_features_stack = torch.Tensor(
                self.pca_model.transform(wsi_features_stack.numpy()))
        
        ## Patient average (over slides):
        wsi_features_avg = torch.mean(wsi_features_stack, dim=0)
        ...
        return (
            ...
            wsi_features_avg, torch.zeros(1), ## patches masks
            clin_features_list, torch.zeros(1), ## omics pways data
