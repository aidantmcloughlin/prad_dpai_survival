...
class CMTAGleason(nn.Module):
    def __init__(self, params):
        super(CMTAGleason, self).__init__()
        ...
        ## Gleason attention.
        self.P_Gl_Att = MultiheadAttention(embed_dim=hidden[-1], num_heads=1)
        self.G_Gl_Att = MultiheadAttention(embed_dim=hidden[-1], num_heads=1)
    ...
    def forward(self, **kwargs):
        ...
        pathomics_in_gleason, Att = self.P_Gl_Att(
        ...
        genomics_in_gleason, Att = self.G_Gl_Att(
            gleason_features.transpose(1, 0),
            patch_token_genomics_encoder.transpose(1, 0),
            patch_token_genomics_encoder.transpose(1, 0),
        ) ## pathway-dimensional token-output
        ...
        if self.fusion == "concat":
            fused_rep = torch.concat((
                ...
                (genomics_in_gleason.squeeze(0) + pathomics_in_gleason.squeeze(0)) / 2,
                ), dim=1)
            # logits = self.mm(
        ...
        other_outs_dict = {
            ...
            'P_hat_gleason': pathomics_in_gleason.squeeze(0),
            'G': cls_token_genomics_encoder,
            'G_hat': cls_token_genomics_decoder_path,
            'G_hat_gleason': genomics_in_gleason.squeeze(0),
