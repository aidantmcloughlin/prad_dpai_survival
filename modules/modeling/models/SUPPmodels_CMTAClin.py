...
class CMTAClin(nn.Module):
    def __init__(self, params):
        super(CMTAClin, self).__init__()
        ...
        ### Gleason Embedding Network
        gleason_embed_size = self.size_dict["pathomics"][self.model_size][-1]
        self.gleason_encoder = nn.Embedding(5, gleason_embed_size)
        ...
        if self.fusion == "concat":
            self.mm = nn.Sequential(
                *[
                    nn.Linear(hidden[-1] * 3, hidden[-1]), nn.ReLU(), nn.Dropout(p=0.1),
                    nn.Linear(hidden[-1], hidden[-1]), nn.ReLU(), nn.Dropout(p=0.1)
                ]
            )
    ...
    def forward(self, **kwargs):
        ...
        x_gleason = kwargs['clin_data_list'][1]
        
        # Enbedding
        # gleason embedding:
        gleason_features = self.gleason_encoder(x_gleason).unsqueeze(0)
        # genomics embedding
        ...
        pathomics_in_genomics, Att = self.P_in_G_Att(
            ...
            patch_token_genomics_encoder.transpose(1, 0),
        ) ## patch-dimensional token-output
        pathomics_in_gleason, Att = self.P_in_G_Att(
            patch_token_pathomics_encoder.transpose(1, 0),
            gleason_features.transpose(1, 0),
            gleason_features.transpose(1, 0),
        ) ## patch-dimensional token-output
        genomics_in_pathomics, Att = self.G_in_P_Att(
            ...
            patch_token_pathomics_encoder.transpose(1, 0),
            patch_token_pathomics_encoder.transpose(1, 0),
        ) ## pathway-dimensional token-output
        genomics_in_gleason, Att = self.G_in_P_Att(
            patch_token_genomics_encoder.transpose(1, 0),
            gleason_features.transpose(1, 0),
            gleason_features.transpose(1, 0),
        ) ## pathway-dimensional token-output
        ...
        ## decoder
        ## pathomics decoder
        cls_token_pathomics_decoder_gen, _ = self.pathomics_decoder(
            pathomics_in_genomics.transpose(1, 0))  # cls token + patch tokens
        cls_token_pathomics_decoder_gleason, _ = self.pathomics_decoder(
            pathomics_in_gleason.transpose(1, 0))  # cls token + patch tokens
        ## genomics decoder
        cls_token_genomics_decoder_path, _ = self.genomics_decoder(
            genomics_in_pathomics.transpose(1, 0))  # cls token + patch tokens
        cls_token_genomics_decoder_gleason, _ = self.genomics_decoder(
            genomics_in_gleason.transpose(1, 0))  # cls token + patch tokens

        ### fusion + prediction
        if self.fusion == "concat":
            fused_rep = torch.concat((
                (cls_token_pathomics_encoder + cls_token_pathomics_decoder_gen) / 2,
                (cls_token_genomics_encoder + cls_token_genomics_decoder_path) / 2,
                (cls_token_pathomics_decoder_gleason + cls_token_genomics_decoder_gleason) / 2,), dim=1)
            # logits = self.mm(
            #     torch.concat((fused_rep, x_clin.squeeze()), dim=1)
        ...
        other_outs_dict = {
            ...
            'P_hat': cls_token_pathomics_decoder_gen,
            'P_hat_gleason': cls_token_pathomics_decoder_gleason,
            'G': cls_token_genomics_encoder,
            'G_hat': cls_token_genomics_decoder_path,
            'G_hat_gleason': cls_token_genomics_decoder_gleason,
            'gleason_token': gleason_features.squeeze(0),
    ...
    def captum(self, x_path, clin_data_list, *inputs):
        ...
        x_gleason = clin_data_list[1]

        genomics_features = inputs

        ## expand other inputs to number of examples from x_omic:
        x_path = x_path.repeat(genomics_features[0].shape[0], 1, 1)
        clin_data = clin_data.repeat(genomics_features[0].shape[0], 1,)
        x_gleason = x_gleason.repeat(genomics_features[0].shape[0], 1,)

        ### Enbeddings
        # gleason embedding
        gleason_features = self.gleason_encoder(x_gleason)
        ...
        if self.fusion == "concat":
            fused_rep = torch.concat((
                (cls_token_pathomics_encoder + cls_token_pathomics_decoder_gen + cls_token_pathomics_decoder_gleason) / 3,
                (cls_token_genomics_encoder + cls_token_genomics_decoder_path + cls_token_genomics_decoder_gleason) / 3,), dim=1)
            fused_rep = self.mm(fused_rep)
            all_features = torch.concat((
                fused_rep, 
                clin_data
        ...
        #     fused_rep = self.mm(
        #         (cls_token_pathomics_encoder + cls_token_pathomics_decoder_gen + cls_token_pathomics_decoder_gleason) / 3,
        #         (cls_token_genomics_encoder + cls_token_genomics_decoder_path + cls_token_genomics_decoder_gleason) / 3,
        #     )  # take cls token to make prediction
        #     all_features = torch.concat((
        #         fused_rep, 
        #         clin_data
    ...
    def captum_gene(self, x_path, clin_data_list, *inputs):
        ...
        x_gleason = x_gleason.repeat(x_omic_list[0].shape[0], 1,)
