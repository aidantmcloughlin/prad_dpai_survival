...
if __name__ == '__main__':
    ...
    ### Args to determine where are the paths:
    parser = argparse.ArgumentParser(description='Make Graphs')
    parser.add_argument('--patch_size', type=int, default=1024)
    parser.add_argument('--encoder', type=str, default='imagenet')
    
    args = parser.parse_args() 

    embedding_sub_dir = os.path.join(
        EMBED_ROOT_DIR, 
        str(args.patch_size) + '_' + '40',
        args.encoder)
    h5_path = os.path.join(
        embedding_sub_dir,
        '_2_features/h5_files/')
